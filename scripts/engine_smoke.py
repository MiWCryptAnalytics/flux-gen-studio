"""Real engine smoke test: load a model and render, no GUI involved.
...
Usage: .venv/bin/python scripts/engine_smoke.py <out_dir> [--full]
           [--model flux1|flux2] [--quant bf16|nf4|int8]
"""
...
from fluxstudio.engine import (
    DEFAULT_MODEL, MODELS, GenRequest, Timings, load_pipeline, run_generation,
)
...
def main() -> int:
    ...
    loaded = load_pipeline(
        progress=lambda m: print(f"[load] {m}", flush=True), quant=quant, model=model
    )
    print(f"[load] done in {perf_counter() - t0:.0f}s · {spec.label} · "
          f"{loaded.device_label} · offline={loaded.offline}", flush=True)

    # 1. Tiny render exercises the full path fast.
    request = GenRequest(prompt="a red cube on a white table", width=512,
                         height=512, steps=4, guidance=spec.guidance)
    t0 = perf_counter()
    ...
    request = GenRequest(
        ...
        width=1024, height=1024, steps=steps, guidance=spec.guidance,
    )
    timings = Timings()
    image = run_generation(
        loaded.pipe, request, seed=7,
        on_step=lambda s: print(f"[gen] step {s}/{steps}", flush=True) if s % 7 == 0 else None,
        cancelled=lambda: False,
        timings=timings,
    )
    ...
    image.save(full)
    print(f"[gen] 1024×1024·{steps} steps in {timings.total:.1f}s -> {full}", flush=True)
